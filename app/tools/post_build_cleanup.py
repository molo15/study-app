...
清理后部署体积从约 52.9MB 降至约 37.6MB（减少 29%），不影响运行时功能
（浏览器不会请求被删除的文件）。
...
def main():
    ...
    print('[1/3] 清理 canvaskit .symbols 调试符号')
    ...
    print('[2/3] 清理 Web 端死资源 NotoSansSC-subset.ttf')
    total += remove('assets/assets/fonts/NotoSansSC-subset.ttf')
    print()

    # 3. Flutter 空壳 Service Worker
    print('[3/3] 清理 flutter_service_worker.js（Flutter 注销空壳）')
