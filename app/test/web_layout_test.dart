...
import 'package:quiz_app/ui/widgets/floating_tab_bar.dart';
...
void main() {
  ...
  for (final (name, size) in viewports) {
    testWidgets('$name：导航形态随断点切换（手机/平板 dock / 桌面侧边栏）', (tester) async {
      ...
      if (layout == AppLayout.expanded) {
        ...
        // 手机 / 平板：底部 dock 固定贴底
        final nav = tester.widget<FloatingTabBar>(find.byType(FloatingTabBar));
        expect(nav, isNotNull);
        final rect = tester.getRect(find.byType(FloatingTabBar));
        expect(rect.bottom, greaterThan(size.height - 40),
            reason: '$name 导航栏应在视口底部，实际 bottom=${rect.bottom}');
        expect(find.byType(AppSidebar), findsNothing,
            reason: '$name 形态不显示侧边栏');
      }
    });
  }
}
