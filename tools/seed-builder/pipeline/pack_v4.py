# -*- coding: utf-8 -*-
"""v4 打包：formatVersion=4 —— knowledge 树 + overviews 进 manifest，选择题 answer 改文本编码。

基于 pack_v010.py 扩展：
- manifest 新增 knowledge（知识点树，含每点基础题数 questionCount）与 overviews（每章概览）
- 选择题 answer 编码：洗牌后存正确项文本（single=字符串，multi=文本数组）；
  App 端 fromBankJson 映射回 key（兼容新旧包）
- 版本 0.11.0（v4 格式，与 v3 的 0.10.0 区分）
"""
import json, os, random, re, shutil, zipfile
from collections import Counter

BASE = r"D:\study_app\tools\seed-builder"
OUT_DIR = os.path.join(BASE, "out", "packages", "v011")
...
REPORT = os.path.join(BASE, "out", "reports", "pack_report_v011.md")
VERSION = "0.11.0"
...
def main():
    ...
    report_lines = ["# v0.11.0（formatVersion=4）打包报告", ""]
    summary = []
    ...
    for bank, (name, cn, v09rel) in BANKS.items():
        ...
        manifest = {
            ...
            "name": name,
            "version": VERSION,
            "generatedAt": "2026-08-27T14:00:00+08:00",
    ...
    report_lines.append("- 版本 0.11.0；已部署 app/assets/banks/ 并移除 v0.10.0。")
